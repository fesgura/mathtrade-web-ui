...
const MyItems = ({ onChangeList, myItemsToOffer }) => {
  ...
  useEffect(() => {
    if (myItemList.length) {
      setMyItemsIdToChangeSelectAll(
        myItemList.length === myItemsToOffer.length
      );
    }
  }, [myItemList, myItemsToOffer]);
  ...
  return errors ? (
    ...
    <div className="min-list-items">
      ...
      {loading ? (
        ...
        myItemList
          .sort((a, b) => {
            const { value, order } = orderByOpt;
            const orderNum = order ? 1 : -1;
            switch (value) {
              case "title":
                return a.title > b.title ? -1 * orderNum : 1 * orderNum;
              case "value":
                return a.value > b.value ? -1 * orderNum : 1 * orderNum;
              default:
                return 0;
            }
          })
